...
shell-can-only-run-from-pre-round-lobby = Você só pode executar este comando enquanto o jogo está no lobby pré-jogo.
...
shell-must-have-body = Você deve ter um corpo para executar este comando.

## Arguments

shell-need-exactly-one-argument = Exatamente um argumento é necessário.
shell-wrong-arguments-number-need-specific = São necessários {$properAmount} argumentos, mas havia {$currentAmount}.
...
shell-need-between-arguments = São necessários {$lower} a {$upper} argumentos!
shell-need-minimum-arguments = São necessários pelo menos {$minimum} argumentos!
shell-need-minimum-one-argument = É necessário pelo menos um argumento!
...
shell-invalid-color-hex = Cor hexadecimal inválida!
...
shell-timespan-minutes-must-be-correct = {$span} não é um intervalo de tempo válido em minutos.
