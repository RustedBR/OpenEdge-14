...
public abstract partial class OE14SharedCookingSystem : EntitySystem
{
    ...
    /// </summary>
    private readonly ProtoId<ReagentPrototype> _burntFoodReagent = "OE14BurntFood";
    ...
    /// </summary>
    protected List<OE14CookingRecipePrototype> OrderedRecipes = [];
    ...
    public override void Initialize()
    {
        ...
        SubscribeLocalEvent<OE14FoodHolderComponent, ExaminedEvent>(OnExaminedEvent);
    }
    ...
    {
        OrderedRecipes = _proto.EnumeratePrototypes<OE14CookingRecipePrototype>()
    ...
    private void OnExaminedEvent(Entity<OE14FoodHolderComponent> ent, ref ExaminedEvent args)
    {
        ...
        args.PushMarkup(Loc.GetString("oe14-cooking-examine",
    ...
    /// </summary>
    protected virtual bool TryTransferFood(Entity<OE14FoodHolderComponent> target,
        Entity<OE14FoodHolderComponent> source)
    {
        ...
        if (_solution.TryGetSolution(target.Owner, holderFoodComp.Solution, out var holderSoln, out var solution))
        {
            if (solution.Volume > 0)
            {
                _popup.PopupEntity(Loc.GetString("oe14-cooking-popup-not-empty", ("name", MetaData(target).EntityName)),
                    target);
                return false;
            }
    ...
    private void UpdateFoodDataVisuals(
        Entity<OE14FoodHolderComponent> ent,
        bool rename = true)
    {
        ...
        bool rename = true)
    {
        ...
        //Visuals
        ent.Comp.FoodData = new OE14FoodData(data);
        ...
        DirtyField(ent, ent.Comp, nameof(OE14FoodHolderComponent.FoodData));

        //Sliceable
        // > on server overrided side
    }

    public OE14CookingRecipePrototype? GetRecipe(Entity<OE14FoodCookerComponent> ent)
    {
    ...
    public OE14CookingRecipePrototype? GetRecipe(ProtoId<OE14FoodTypePrototype> foodType,
        Solution? solution,
        List<ProtoId<TagPrototype>> allTags)
    {
        if (OrderedRecipes.Count == 0)
        {
            throw new InvalidOperationException(
                "No cooking recipes found. Please ensure that the OE14CookingRecipePrototype is defined and loaded.");
        }

        OE14CookingRecipePrototype? selectedRecipe = null;
    ...
    protected void CreateFoodData(Entity<OE14FoodCookerComponent> ent, OE14CookingRecipePrototype recipe)
    {
        ...
        var newData = new OE14FoodData(recipe.FoodData);
        ...
        if (TryComp<OE14FoodHolderComponent>(ent.Owner, out var holder))
        {
    ...
    protected void BurntFood(Entity<OE14FoodCookerComponent> ent)
    {
        if (!TryComp<OE14FoodHolderComponent>(ent, out var holder) || holder.FoodData is null)
            return;
        ...
        holder.FoodData.Name = Loc.GetString("oe14-meal-recipe-burned-trash-name");
        holder.FoodData.Desc = Loc.GetString("oe14-meal-recipe-burned-trash-desc");
        ...
        DirtyField(ent.Owner, holder, nameof(OE14FoodHolderComponent.FoodData));
    }
}
