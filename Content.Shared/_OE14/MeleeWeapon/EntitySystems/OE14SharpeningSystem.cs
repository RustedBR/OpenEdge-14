...
public sealed class OE14SharpeningSystem : EntitySystem
{
    ...
    public override void Initialize()
    {
        ...
        SubscribeLocalEvent<OE14SharpenedComponent, GetMeleeDamageEvent>(OnGetMeleeDamage,
            after: [typeof(SharedWieldableSystem)]);
        SubscribeLocalEvent<OE14SharpenedComponent, ExaminedEvent>(OnExamined);
        SubscribeLocalEvent<OE14SharpenedComponent, MeleeHitEvent>(OnMeleeHit);

        SubscribeLocalEvent<OE14SharpeningStoneComponent, AfterInteractEvent>(OnAfterInteract);
        SubscribeLocalEvent<OE14SharpeningStoneComponent, ActivateInWorldEvent>(OnInteract);
    }

    public static void ReduceSharpness(Entity<OE14SharpenedComponent> ent, DamageSpecifier dmg)
    {
        ent.Comp.Sharpness =
            MathHelper.Clamp(ent.Comp.Sharpness - dmg.GetTotal().Float() * ent.Comp.SharpnessDamageBy1Damage, 0.1f, 1f);
    }

    private static void OnMeleeHit(Entity<OE14SharpenedComponent> sharpened, ref MeleeHitEvent args)
    {
    ...
    private void OnInteract(Entity<OE14SharpeningStoneComponent> stone, ref ActivateInWorldEvent args)
    {
        if (args.Handled)
            return;
        ...
        {
            if (!TryComp<OE14SharpenedComponent>(item, out var sharpened))
                continue;
    ...
    private void OnAfterInteract(Entity<OE14SharpeningStoneComponent> stone, ref AfterInteractEvent args)
    {
        if (!args.CanReach || args.Target == null || !TryComp<OE14SharpenedComponent>(args.Target, out var sharpened))
            return;
    ...
    private void SharpThing(Entity<OE14SharpeningStoneComponent> stone,
        EntityUid target,
        OE14SharpenedComponent component,
        EntityUid user)
    {
    ...
    private void OnExamined(Entity<OE14SharpenedComponent> sharpened, ref ExaminedEvent args)
    {
    ...
    private static void OnGetMeleeDamage(Entity<OE14SharpenedComponent> sharpened, ref GetMeleeDamageEvent args)
    {
