...
using Content.Shared._OE14.Skill.Restrictions;
...
public abstract partial class OE14SharedSkillSystem : EntitySystem
{
    ...
    private EntityQuery<OE14SkillStorageComponent> _skillStorageQuery;

    public override void Initialize()
    {
        base.Initialize();

        _skillStorageQuery = GetEntityQuery<OE14SkillStorageComponent>();

        SubscribeLocalEvent<OE14SkillStorageComponent, MapInitEvent>(OnMapInit);
        SubscribeLocalEvent<OE14SkillStorageComponent, ComponentInit>(OnComponentInit);
        SubscribeLocalEvent<OE14SkillPointConsumableComponent, UseInHandEvent>(OnInteracted);
    ...
    private void OnInteracted(Entity<OE14SkillPointConsumableComponent> ent, ref UseInHandEvent args)
    {
    ...
    {
        // DISABLED: This was causing duplicate skill application.
        // The OnComponentInit handler already applies free skills correctly.
        // If both were active, skills were applied twice (once in ComponentInit, once in MapInit after clearing).
        // See: https://github.com/RustedBR/OpenEdge-14/issues/XXX
    }

    private void OnComponentInit(Entity<OE14SkillStorageComponent> ent, ref ComponentInit args)
    {
        // Apply free learned skills (innate skills like Elf's Metamagic T1/T2)
        // This ensures stat modifiers are applied even if MapInitEvent already fired
        ...
        foreach (var skill in free)
        {
            ...
            {
                // Only apply if not already in LearnedSkills
                if (_proto.TryIndex(skill, out var indexedSkill))
                {
                    foreach (var effect in indexedSkill.Effects)
                    {
                        effect.AddSkill(EntityManager, ent.Owner);
                    }
    ...
    public void AddSkillTree(EntityUid target,
        ProtoId<OE14SkillTreePrototype> tree,
        OE14SkillStorageComponent? component = null)
    {
        if (!Resolve(target, ref component, false))
            return;

        component.AvailableSkillTrees.Add(tree);
        DirtyField(target, component, nameof(OE14SkillStorageComponent.AvailableSkillTrees));
    }
    ...
    public bool TryAddSkill(EntityUid target,
        ProtoId<OE14SkillPrototype> skill,
        OE14SkillStorageComponent? component = null,
        bool free = false)
    {
        if (!Resolve(target, ref component, false))
        ...
        var learnEv = new OE14SkillLearnedEvent(skill, target);
    ...
    /// </summary>
    public string GetSkillName(ProtoId<OE14SkillPrototype> skill)
    {
        if (!_proto.TryIndex(skill, out var indexedSkill))
            return string.Empty;
    ...
    /// </summary>
    public string GetSkillDescription(ProtoId<OE14SkillPrototype> skill)
    {
        if (!_proto.TryIndex(skill, out var indexedSkill))
            return string.Empty;
    ...
    /// </summary>
    public HashSet<ProtoId<OE14SkillPrototype>> GetFrontierSkills(EntityUid target,
        OE14SkillStorageComponent? component = null)
    {
        var skills = new HashSet<ProtoId<OE14SkillPrototype>>();
        if (!Resolve(target, ref component, false))
            return skills;
    ...
    public void AddSkillPoints(EntityUid target,
        ProtoId<OE14SkillPointPrototype> type,
        FixedPoint2 points,
        ...
        OE14SkillStorageComponent? component = null)
    {
        ...
        {
            skillContainer = new OE14SkillPointContainerEntry();
        ...
        DirtyField(target, component, nameof(OE14SkillStorageComponent.SkillPoints));
...
[ByRefEvent]
public record struct OE14SkillLearnedEvent(ProtoId<OE14SkillPrototype> Skill, EntityUid User);
