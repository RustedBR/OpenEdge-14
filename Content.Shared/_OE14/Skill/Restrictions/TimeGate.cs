...
public sealed partial class TimeGate : OE14SkillRestriction
{
    ...
    public override bool Check(IEntityManager entManager, EntityUid target)
    {
        ...
        if (cfg.GetCVar(CCVars.OE14SkillTimers) == false)
            return true;
    ...
    public override string GetDescription(IEntityManager entManager, IPrototypeManager protoManager)
    {
        ...
        if (cfg.GetCVar(CCVars.OE14SkillTimers) == false)
            return Loc.GetString("oe14-skill-req-timegate-disabled", ("minute", Minutes));

        return Loc.GetString("oe14-skill-req-timegate", ("minute", Minutes), ("left", Math.Ceiling(leftoverTime.TotalMinutes)));
    }
}
