...
public abstract class OE14SharedMagicVisionSystem : EntitySystem
{
    ...
    public readonly EntProtoId MagicTraceProto = "OE14MagicVisionMarker";

    public override void Initialize()
    {
        base.Initialize();

        SubscribeLocalEvent<OE14MagicVisionMarkerComponent, ExaminedEvent>(OnExamined);
        SubscribeLocalEvent<OE14MagicVisionStatusEffectComponent, StatusEffectRelayedEvent<MobStateChangedEvent>>(OnMobStateChange);
    }

    private void OnMobStateChange(Entity<OE14MagicVisionStatusEffectComponent> ent, ref StatusEffectRelayedEvent<MobStateChangedEvent> args)
    {
    ...
    protected virtual void OnExamined(Entity<OE14MagicVisionMarkerComponent> ent, ref ExaminedEvent args)
    {
        ...
        sb.Append($"{Loc.GetString("oe14-magic-vision-timed-past")} {timePassed.Minutes}:{timePassed.Seconds:D2}\n");
        ...
        {
            sb.Append($"{Loc.GetString("oe14-magic-vision-aura")} {imprint}");
            args.AddMarkup(sb.ToString());
            return;
        }
        ...
        sb.Append($"{Loc.GetString("oe14-magic-vision-aura")} {obscuredImprint}");

        args.AddMarkup(sb.ToString());
    }

    public string? GetAuraImprint(Entity<OE14AuraImprintComponent?> ent)
    {
        if (!Resolve(ent, ref ent.Comp))
            return null;

        if (_statusEffects.TryEffectsWithComp<OE14HideMagicAuraStatusEffectComponent>(ent, out var hideComps))
        {
        ...
        EntityCoordinates? target = null)
    {
        ...
        var markerComp = EnsureComp<OE14MagicVisionMarkerComponent>(ent);
