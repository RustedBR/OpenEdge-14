...
/// </summary>
public sealed class OE14DayCycleSystem : EntitySystem
{
    ...
    public override void Update(float frameTime)
    {
        ...
        var query = EntityQueryEnumerator<LightCycleComponent, OE14DayCycleComponent, MapComponent>();
        while (query.MoveNext(out var uid, out var lightCycle, out var dayCycle, out var map))
        {
            ...
            if (oldLightLevel > newLightLevel)
            {
                if (oldLightLevel > dayCycle.Threshold)
                {
                    ...
                    {
                        var ev = new OE14StartNightEvent(uid);
                        RaiseLocalEvent(ev);
                    }
                }
            }
            ...
            if (oldLightLevel < newLightLevel)
            {
                if (oldLightLevel < dayCycle.Threshold)
                {
                    if (newLightLevel > dayCycle.Threshold)
                    {
                        var ev = new OE14StartDayEvent(uid);
                        RaiseLocalEvent(ev);
                    }
                }
            }
...
public sealed class OE14StartNightEvent(EntityUid map) : EntityEventArgs
{
    public EntityUid Map = map;
}

public sealed class OE14StartDayEvent(EntityUid map) : EntityEventArgs
{
    public EntityUid Map = map;
}
...
internal sealed class CheckLightLevel : LocalizedCommands
{
    ...
    public override string Command => "oe14_check_light";
    ...
    public override void Execute(IConsoleShell shell, string argStr, string[] args)
    {
        ...
        var dayCycle = _entityManager.GetEntitySystem<OE14DayCycleSystem>();
