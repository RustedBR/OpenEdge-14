...
public sealed class OE14WallmountSystem : EntitySystem
{
    ...
    public override void Initialize()
    {
        ...
        SubscribeLocalEvent<OE14WallmountedComponent, ComponentShutdown>(OnWallmountShutdown);
        SubscribeLocalEvent<OE14WallmountedComponent, AnchorStateChangedEvent>(OnWallmountAnchorChanged);
    }

    private void OnWallmountAnchorChanged(Entity<OE14WallmountedComponent> ent, ref AnchorStateChangedEvent args)
    {
        if (!args.Anchored)
            ClearWallmounts(ent);
    }

    private void OnWallmountShutdown(Entity<OE14WallmountedComponent> ent, ref ComponentShutdown args)
    {
        ClearWallmounts(ent);
    }
    ...
    public override void Update(float frameTime)
    {
        ...
        var query = EntityQueryEnumerator<OE14WallmountComponent>();
        while (query.MoveNext(out var uid, out var wallmount))
        {
            ...
            {
                RemComp<OE14WallmountComponent>(uid);
            }
        }
    }

    private void ClearWallmounts(Entity<OE14WallmountedComponent> ent)
    {
    ...
    private bool TryAttachWallmount(Entity<OE14WallmountComponent> wallmount)
    {
        ...
        foreach (var entt in anchored)
        {
            ...
            EnsureComp<OE14WallmountedComponent>(entt, out var wallmounted);
