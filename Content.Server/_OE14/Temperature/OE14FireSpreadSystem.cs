...
public sealed partial class OE14FireSpreadSystem : OE14SharedFireSpreadSystem
{
    ...
    private readonly EntProtoId _fireProto = "OE14Fire";

    private readonly HashSet<Entity<OE14FireSpreadComponent>> _spreadEnts = new();

    public override void Initialize()
    {
        base.Initialize();

        SubscribeLocalEvent<FlammableComponent, OE14IgnitionDoAfter>(OnFlammableIgnited);
        SubscribeLocalEvent<SmokableComponent, OE14IgnitionDoAfter>(OnDelayedIgnite);
        SubscribeLocalEvent<SmokingPipeComponent, OE14IgnitionDoAfter>(OnDelayedPipeIgnite);
        SubscribeLocalEvent<OE14FlammableBonusDamageComponent, MeleeHitEvent>(OnFlammableMeleeHit);
    }

    private void OnFlammableMeleeHit(Entity<OE14FlammableBonusDamageComponent> ent, ref MeleeHitEvent args)
    {
    ...
    private void OnFlammableIgnited(Entity<FlammableComponent> ent, ref OE14IgnitionDoAfter args)
    {
    ...
    //For smokable cigars
    private void OnDelayedIgnite(Entity<SmokableComponent> ent, ref OE14IgnitionDoAfter args)
    {
        _smoking.SetSmokableState(ent, SmokableState.Lit, ent.Comp);
    }

    //For smokable pipes
    private void OnDelayedPipeIgnite(Entity<SmokingPipeComponent> pipe, ref OE14IgnitionDoAfter args)
    {
    ...
    {
        var query = EntityQueryEnumerator<OE14AutoIgniteComponent, FlammableComponent>();
        while (query.MoveNext(out var uid, out var autoIgnite, out var flammable))
        {
            ...
            RemCompDeferred<OE14AutoIgniteComponent>(uid);
        }
    }
    ...
    private void UpdateFireSpread()
    {
        ...
        var query = EntityQueryEnumerator<OE14ActiveFireSpreadingComponent, OE14FireSpreadComponent, FlammableComponent, TransformComponent>();
        while (query.MoveNext(out var uid, out _, out var spread, out var flammable, out var xform))
        {
            ...
            _spreadEnts.Add(new Entity<OE14FireSpreadComponent>(uid, spread));
        }
    ...
    private void IgniteEntities(Entity<OE14FireSpreadComponent> spread)
    {
    ...
    private void IgniteTiles(Entity<OE14FireSpreadComponent> spread)
    {
