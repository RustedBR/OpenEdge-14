...
namespace Content.Server._OE14.Passport;

public sealed partial class OE14PassportSystem : EntitySystem
{
    ...
    public readonly EntProtoId PassportProto = "OE14Passport";
    ...
    private void OnPlayerSpawning(PlayerSpawnCompleteEvent ev)
    {
        ...
        _paper.TryStamp((passport, paper),
            new StampDisplayInfo
            {
                ...
                StampedName = Loc.GetString("oe14-passport-stamp")
            },
            "");
    ...
    private string GeneratePassportText(PlayerSpawnCompleteEvent ev)
    {
        ...
        //Name
        sb.AppendLine(Loc.GetString("oe14-passport-name", ("name",  ev.Profile.Name)));
        //Species
        if (_proto.TryIndex(ev.Profile.Species, out var indexedSpecies))
            sb.AppendLine(Loc.GetString("oe14-passport-species", ("species", Loc.GetString(indexedSpecies.Name))));
        //Birthday
        var birthday = $"{_random.Next(40)}.{_random.Next(12)}.{225 - ev.Profile.Age}";
        sb.AppendLine(Loc.GetString("oe14-passport-birth-date", ("birthday",  birthday)));
        //Job
        if (ev.JobId is not null && _proto.TryIndex<JobPrototype>(ev.JobId, out var indexedJob))
            sb.AppendLine(Loc.GetString("oe14-passport-job", ("job",  Loc.GetString(indexedJob.Name))));
