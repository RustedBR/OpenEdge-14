...
public sealed class OE14WeatherEffectSystem : EntitySystem
{
    ...
    private readonly List<(OE14WeatherEffectJob Job, CancellationTokenSource CancelToken)> _weatherJobs = new();
        ...
        while (query.MoveNext(out var mapUid, out var weather, out var mapComp))
        {
            foreach (var (proto, _) in weather.Weather)
            {
                ...
                foreach (var config in indexedWeather.Config)
                {
                    ...
                    var job = new OE14WeatherEffectJob(
                        JobMaxTime,
                        EntityManager,
                        _lookup,
                        _weather,
