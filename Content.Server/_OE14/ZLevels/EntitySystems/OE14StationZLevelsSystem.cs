...
public sealed partial class OE14StationZLevelsSystem : EntitySystem
{
    ...
    public override void Initialize()
    {
        ...
        SubscribeLocalEvent<OE14StationZLevelsComponent, StationPostInitEvent>(OnStationPostInit);
    }

    private void OnStationPostInit(Entity<OE14StationZLevelsComponent> ent, ref StationPostInitEvent args)
    {
        ...
        {
            Log.Error($"Failed to init OE14StationZLevelsSystem: defaultMap is null");
            return;
        }
        ...
        foreach (var (map, level) in ent.Comp.Levels)
        {
            ...
            {
                Log.Error($"Key duplication for OE14StationZLevelsSystem at level {map}!");
                continue;
            }

            if (level.Path is null)
            {
                Log.Error($"path {level.Path.ToString()} for OE14StationZLevelsSystem at level {map} don't exist!");
                continue;
            }
            ...
            {
                Log.Error($"Failed to load map for OE14StationZLevelsSystem at level {map}!");
                continue;
            }

            Log.Info($"Created map {mapEnt.Value.Comp.MapId} for OE14StationZLevelsSystem at level {map}");
    ...
    {
        var query = EntityQueryEnumerator<OE14StationZLevelsComponent>();
