...
public sealed partial class OE14DemiplaneSystem : OE14SharedDemiplaneSystem
{
    ...
    public override void Initialize()
    {
        ...
        SubscribeLocalEvent<OE14DemiplaneRiftComponent, InteractHandEvent>(OnRiftInteracted);
    }
    ...
    public override void Update(float frameTime)
    {
        ...
        var query = EntityQueryEnumerator<OE14DemiplaneRiftComponent>();
        while (query.MoveNext(out var uid, out var demiplaneRift))
        {
            ...
            var targetQuery = EntityQueryEnumerator<OE14DemiplaneEnterPointComponent>();
    ...
    private void OnRiftInteracted(Entity<OE14DemiplaneRiftComponent> ent, ref InteractHandEvent args)
    {
        if (HasComp<OE14DemiplaneBlockInteractionsComponent>(args.User))
            return;
        ...
        {
            _sawmill.Error($"Station {station} does not have a OE14StationDemiplaneMapComponent!");
            QueueDel(ent);
            return;
        }
        ...
        if (TryComp<OE14DemiplaneMapComponent>(originMap, out var originDemiplaneComp))
        ...
        EnsureComp<OE14DemiplaneMapComponent>(mapUid).Position = targetPosition.Value;
    ...
    /// </summary>
    public OE14ProceduralLocationPrototype SelectLocation(int level)
    {
        OE14ProceduralLocationPrototype? selectedConfig = null;

        HashSet<OE14ProceduralLocationPrototype> suitableConfigs = new();
        foreach (var locationConfig in _proto.EnumeratePrototypes<OE14ProceduralLocationPrototype>())
        {
    ...
    /// </summary>
    public List<ProtoId<OE14ProceduralModifierPrototype>> SelectModifiers(
        int level,
        OE14ProceduralLocationPrototype location,
        Dictionary<ProtoId<OE14ProceduralModifierCategoryPrototype>, float> modifierLimits)
    {
        List<ProtoId<OE14ProceduralModifierPrototype>> selectedModifiers = new();

        //Modifier generation
        Dictionary<OE14ProceduralModifierPrototype, float> suitableModifiersWeights = new();
        foreach (var modifier in _proto.EnumeratePrototypes<OE14ProceduralModifierPrototype>())
        {
            var passed = true;
        ...
        //Limits calculation
        Dictionary<ProtoId<OE14ProceduralModifierCategoryPrototype>, float> limits = new();
    ...
    /// </summary>
    private static OE14ProceduralModifierPrototype ModifierPick(
        Dictionary<OE14ProceduralModifierPrototype, float> weights,
        IRobustRandom random)
    {
        ...
        // Shouldn't happen
        throw new InvalidOperationException($"Invalid weighted pick in OE14DemiplanSystem.Generation!");
    }
}

public sealed class OE14LocationGeneratedEvent : EntityEventArgs
{
}
