...
public sealed class OE14CrashingShipRule : GameRuleSystem<OE14CrashingShipRuleComponent>
{
    ...
    {
        base.Initialize();
        _sawmill = _logManager.GetSawmill("oe14_crashing_ship_rule");

        SubscribeLocalEvent<OE14CrashingShipComponent, FTLCompletedEvent>(OnFTLCompleted);
    }
    ...
    protected override void Started(EntityUid uid,
        OE14CrashingShipRuleComponent component,
    ...
    private void OnFTLCompleted(Entity<OE14CrashingShipComponent> ent, ref FTLCompletedEvent args)
    {
        SpawnRandomExplosion(ent, ent.Comp.FinalExplosionProto, 10);
        RemCompDeferred<OE14CrashingShipComponent>(ent);
    }

    private void UpdateExplosions(float frameTime)
    {
        var ruleQuery = EntityQueryEnumerator<OE14CrashingShipRuleComponent>();
        while (ruleQuery.MoveNext(out var uid, out var rule))
        {
            ...
            AddComp<OE14CrashingShipComponent>(rule.Ship.Value);
            rule.PendingExplosions = false;
        }

        var query = EntityQueryEnumerator<OE14CrashingShipComponent>();
    ...
    private void SpawnRandomExplosion(Entity<OE14CrashingShipComponent> grid, EntProtoId explosionProto, int count)
    {
