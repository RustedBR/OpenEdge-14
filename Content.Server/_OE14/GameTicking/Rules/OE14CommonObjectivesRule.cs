...
using Content.Server._OE14.StationCommonObjectives;
...
public sealed class OE14CommonObjectivesRule : GameRuleSystem<OE14CommonObjectivesRuleComponent>
{
    ...
    protected override void Started(EntityUid uid,
        OE14CommonObjectivesRuleComponent component,
        GameRuleComponent gameRule,
        GameRuleStartedEvent args)
    {
        base.Started(uid, component, gameRule, args);

        var query = EntityQueryEnumerator<OE14StationCommonObjectivesComponent>();
    ...
    private void OnPlayerSpawning(PlayerSpawnCompleteEvent args)
    {
        ...
        if (!TryComp<OE14StationCommonObjectivesComponent>(args.Station, out var stationCommonObjectives))
            return;
        ...
        while (query.MoveNext(out var objectives))
        {
            ...
            foreach (var group in grouped)
            {
                ...
                foreach (var objEnt in group.Value)
                {
                    ...
                    var status = "oe14-objective-endtext-status-failure";
                    if (progress > 0.75f)
                        status = "oe14-objective-endtext-status-success-a";
                    if (progress > 0.99f)
                        status = "oe14-objective-endtext-status-success";

                    var meta = MetaData(objEnt);
                    sb.Append($"{Loc.GetString(objComp.LocIssuer)}: {meta.EntityName}\n");
                    sb.Append($"{Loc.GetString("oe14-objective-endtext-progress", ("value", (int)(progress * 100)))} - {Loc.GetString(status)}\n");
                }
