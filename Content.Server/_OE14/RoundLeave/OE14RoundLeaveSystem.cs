...
public sealed class OE14RoundLeaveSystem : EntitySystem
{
    ...
    private EntityQuery<OE14RoundLeavingComponent> _leavingQuery;
    private EntityQuery<ActorComponent> _actorQuery;
    public override void Initialize()
    {
        base.Initialize();

        SubscribeLocalEvent<OE14RoundLeaveComponent, StartCollideEvent>(OnCollide);
        SubscribeLocalEvent<OE14RoundLeaveComponent, EndCollideEvent>(EndCollide);

        SubscribeLocalEvent<OE14RoundLeavingComponent, MapInitEvent>(OnStartLeaving);
        SubscribeLocalEvent<OE14RoundLeavingComponent, MindRemovedMessage>(OnLeaveRound);

        _mindContainerQuery = GetEntityQuery<MindContainerComponent>();
        _leavingQuery = GetEntityQuery<OE14RoundLeavingComponent>();
        _actorQuery = GetEntityQuery<ActorComponent>();
    }

    private void OnCollide(Entity<OE14RoundLeaveComponent> ent, ref StartCollideEvent args)
    {
        if (!_mindContainerQuery.HasComp(args.OtherEntity))
            return;

        EnsureComp<OE14RoundLeavingComponent>(args.OtherEntity, out var leaving);
    ...
    private void EndCollide(Entity<OE14RoundLeaveComponent> ent, ref EndCollideEvent args)
    {
        if (_leavingQuery.TryGetComponent(args.OtherEntity, out var leaving))
        {
            ...
            {
                RemComp<OE14RoundLeavingComponent>(args.OtherEntity);
            }
        }
    }

    private void OnStartLeaving(Entity<OE14RoundLeavingComponent> ent, ref MapInitEvent args)
    {
        var msg = Loc.GetString("oe14-earlyleave-warning");
    ...
    private void OnLeaveRound(Entity<OE14RoundLeavingComponent> ent, ref MindRemovedMessage args)
    {
    ...
    private void LeaveRound(EntityUid uid, NetUserId userId, Entity<MindComponent>? mind = null)
    {
        ...
        //    Loc.GetString(
        //        _mobState.IsAlive(uid) ? "oe14-earlyleave-ship-announcement" : "oe14-earlyleave-ship-announcement-dead",
        //        ("character", name),
        //        ("job", CultureInfo.CurrentCulture.TextInfo.ToTitleCase(jobName))
        //    ),
        //    Loc.GetString("oe14-ship-sender"),
