...
public sealed class PlayTimeTrackingSystem : EntitySystem
{
    ...
    public bool IsAllowed(ICommonSession player, string role)
    {
        ...
        return JobRequirements.TryRequirementsMet(player.UserId, job, playTimes, out _, EntityManager, _prototypes, (HumanoidCharacterProfile?) _preferencesManager.GetPreferences(player.UserId).SelectedCharacter); //OE14 add NetUserId for sponsorship checks
    }
    ...
    public HashSet<ProtoId<JobPrototype>> GetDisallowedJobs(ICommonSession player)
    {
        ...
        {
            if (JobRequirements.TryRequirementsMet(player.UserId, job, playTimes, out _, EntityManager, _prototypes, (HumanoidCharacterProfile?) _preferencesManager.GetPreferences(player.UserId).SelectedCharacter)) //OE14 add NetUserId for sponsorship checks
                roles.Add(job.ID);
        }
