...
{
    [Dependency] private readonly WeatherSystem _weather = default!; //OE14
    ...
    private void OnEvaporationMapInit(Entity<EvaporationComponent> entity, ref MapInitEvent args)
    {
        ...
        //OE14 Force evaporation under sky
        var xform = Transform(entity);
        if (TryComp<MapGridComponent>(xform.GridUid, out var mapGrid))
        {
            var tileRef = _map.GetTileRef(xform.GridUid.Value, mapGrid, xform.Coordinates);
            entity.Comp.OE14ForceEvaporation = _weather.CanWeatherAffect(xform.GridUid.Value, mapGrid, tileRef);
        }
        //OE14 End force evaporation under sky
    }
    ...
    private void TickEvaporation()
    {
        ...
        while (query.MoveNext(out var uid, out var evaporation, out var puddle))
        {
            ...
            //OE14 force evaporation under sky
            if (evaporation.OE14ForceEvaporation)
            {
                var reagentTick = evaporation.EvaporationAmount * EvaporationCooldown.TotalSeconds;
                puddleSolution.SplitSolution(reagentTick);
            }
            //OE14 force evaporation under sky end
