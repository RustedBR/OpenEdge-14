...
public sealed class StationRecordsSystem : SharedStationRecordsSystem
{
        ...
        string? jobId, StationRecordsComponent records)
    {
        ...
        //if (!_inventory.TryGetSlotEntity(player, "id", out var idUid)) //OE14 - dont have id, but need
        //    return;

        TryComp<FingerprintComponent>(player, out var fingerprintComponent);
        TryComp<DnaComponent>(player, out var dnaComponent);

        CreateGeneralRecord(station, null/*OE14 null*/, profile.Name, profile.Age, profile.Species, profile.Gender, jobId, fingerprintComponent?.Fingerprint, dnaComponent?.DNA, profile, records);
    }
