...
public sealed partial class DocumentParsingManager
{
    ...
    public void Initialize()
    {
        ...
        // CodeBlock uses a raw-text content parser to preserve newlines and special characters
        // (C# generics, braces, brackets, etc.) without escaping.
        _tagControlParsers.Add("CodeBlock", CreateCodeBlockParser());
    ...
    // Parser for <CodeBlock>...</CodeBlock>: reads raw text preserving all characters.
    private static Parser<char, Control> CreateCodeBlockParser()
    {
        var rawContent = OneOf(
            Try(ImmediateTagEnd).ThenReturn(string.Empty),
            TagEnd.Then(
                Any.Until(TryTagTerminator("CodeBlock")).Select(string.Concat)
            )
        );

        return Map(
            (_, content) => (Control) new Richtext.CodeBlock(content),
            ParseTagArgs("CodeBlock"),
            rawContent
        ).Labelled("CodeBlock control");
    }
