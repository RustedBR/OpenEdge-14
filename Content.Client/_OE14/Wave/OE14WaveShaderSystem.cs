...
public sealed class OE14WaveShaderSystem : EntitySystem
{
    ...
    public override void Initialize()
    {
        ...
        SubscribeLocalEvent<OE14WaveShaderComponent, ComponentStartup>(OnStartup);
        SubscribeLocalEvent<OE14WaveShaderComponent, ComponentShutdown>(OnShutdown);
        SubscribeLocalEvent<OE14WaveShaderComponent, BeforePostShaderRenderEvent>(OnBeforeShaderPost);
    }

    private void OnStartup(Entity<OE14WaveShaderComponent> entity, ref ComponentStartup args)
    {
        entity.Comp.Offset = _random.NextFloat(0, 1000);
        SetShader(entity.Owner, _shader);
    }

    private void OnShutdown(Entity<OE14WaveShaderComponent> entity, ref ComponentShutdown args)
    {
    ...
    private void OnBeforeShaderPost(Entity<OE14WaveShaderComponent> entity, ref BeforePostShaderRenderEvent args)
    {
        if (!_enabled)
            return;
