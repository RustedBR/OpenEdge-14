...
public sealed partial class OE14DemiplaneMapWindow : DefaultWindow
{
    [Dependency] private readonly IPrototypeManager _prototype = default!;
    [Dependency] private readonly ILogManager _log = default!;
    [Dependency] private readonly IClientAdminManager _admin = default!;

    private OE14DemiplaneMapUiState? _cachedState;

    private ISawmill Sawmill { get; init; }

    public OE14DemiplaneMapWindow()
    {
        RobustXamlLoader.Load(this);
        IoCManager.InjectDependencies(this);

        Sawmill = _log.GetSawmill("oe14_demiplane_map_window");
    ...
    public void UpdateState(OE14DemiplaneMapUiState state)
    {
        _cachedState = state;
        ...
        foreach (var node in state.Nodes)
        {
            if (node.Value.Start)
            {
                var startElement = new OE14NodeTreeElement(
                    nodeKey: node.Key.ToString(),
                    gained: true,
                    active: false,
                    node.Value.UiPosition * 100,
                    icon: new SpriteSpecifier.Rsi(new ResPath("_OE14/Interface/NodeTree/demiplane_map.rsi"), "center"));
            ...
            else
            {
                ...
                var treeElement = new OE14NodeTreeElement(
                    nodeKey: node.Key.ToString(),
        ...
        GraphControl.UpdateState(
            new OE14NodeTreeUiState(
                nodeTreeElements,
                edges: edges,
                frameIcon: new SpriteSpecifier.Rsi(new ResPath("/Textures/_OE14/Interface/NodeTree/demiplane_map.rsi"),
                    "frame"),
                hoveredIcon: new SpriteSpecifier.Rsi(
                    new ResPath("/Textures/_OE14/Interface/NodeTree/demiplane_map.rsi"),
                    "hovered"),
    ...
    private void SelectNode(OE14DemiplaneMapNode? node)
    {
