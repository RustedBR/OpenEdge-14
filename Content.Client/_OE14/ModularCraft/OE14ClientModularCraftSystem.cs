...
namespace Content.Client._OE14.ModularCraft;

public sealed class OE14ClientModularCraftSystem : OE14SharedModularCraftSystem
{
    ...
    public override void Initialize()
    {
        ...
        SubscribeLocalEvent<OE14ModularCraftStartPointComponent, AfterAutoHandleStateEvent>(OnAfterHandleState);
        SubscribeLocalEvent<OE14ModularCraftStartPointComponent, GetInhandVisualsEvent>(OnGetInhandVisuals);
        SubscribeLocalEvent<OE14ModularCraftStartPointComponent, GetEquipmentVisualsEvent>(OnGetEquipmentVisuals);
    }

    private void OnAfterHandleState(Entity<OE14ModularCraftStartPointComponent> start,
    ...
    private void UpdateIcon(Entity<OE14ModularCraftStartPointComponent> start, SpriteComponent? sprite = null)
    {
        ...
        foreach (var part in start.Comp.InstalledParts)
        {
            ...
            if (indexedPart.IconSprite is null)
            {
                ...
                var keyCode = $"oe14-modular-icon-layer-{counterPart}-default";
            ...
            else
            {
                ...
                {
                    var keyCode = $"oe14-modular-icon-layer-{counterPart}-{counter}";
                    start.Comp.RevealedLayers.Add(keyCode);
    ...
    private void OnGetInhandVisuals(Entity<OE14ModularCraftStartPointComponent> start, ref GetInhandVisualsEvent args)
    {
        var defaultKey = $"oe14-modular-inhand-layer-{args.Location.ToString().ToLowerInvariant()}";
    ...
    private void OnGetEquipmentVisuals(Entity<OE14ModularCraftStartPointComponent> start,
        ref GetEquipmentVisualsEvent args)
    {
        if (!TryComp(args.Equipee, out InventoryComponent? inventory))
            return;

        var defaultKey = $"oe14-modular-clothing-layer-{args.Slot}";
