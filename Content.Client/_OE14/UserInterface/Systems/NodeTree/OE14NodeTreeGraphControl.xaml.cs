...
public sealed partial class OE14NodeTreeGraphControl : BoxContainer
{
    private const float LocalUIScaleMax = 4f;
    private const float LocalUIScaleMin = 1f;

    private OE14NodeTreeUiState? _state;

    private OE14NodeTreeElement? _hoveredNode;
    private OE14NodeTreeElement? _selectedNode;

    private Dictionary<string, OE14NodeTreeElement> _nodeDict = new();
    ...
    public event Action<OE14NodeTreeElement?>? OnNodeSelected;
    public event Action<Vector2>? OnOffsetChanged;

    public OE14NodeTreeGraphControl()
    {
    ...
    public void UpdateState(OE14NodeTreeUiState state)
    {
        _state = state;
