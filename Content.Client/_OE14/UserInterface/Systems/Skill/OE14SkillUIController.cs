...
using System.Text;
using Content.Client._OE14.Skill;
using Content.Client._OE14.Skill.Ui;
using Content.Client._OE14.UserInterface.Systems.NodeTree;
using Content.Client._OE14.UserInterface.Systems.Skill.Window;
...
namespace Content.Client._OE14.UserInterface.Systems.Skill;

[UsedImplicitly]
public sealed class OE14SkillUIController : UIController, IOnStateEntered<GameplayState>, IOnStateExited<GameplayState>,
    IOnSystemChanged<OE14ClientSkillSystem>
{
    [Dependency] private readonly IPlayerManager _playerManager = default!;
    [Dependency] private readonly IPrototypeManager _proto = default!;
    [Dependency] private readonly IEntityManager _entManager = default!;
    [UISystemDependency] private readonly OE14ClientSkillSystem _skill = default!;

    private OE14SkillWindow? _window;
    private EntityUid? _targetPlayer;

    private IEnumerable<OE14SkillPrototype> _allSkills = [];

    private OE14SkillPrototype? _selectedSkill;
    private OE14SkillTreePrototype? _selectedSkillTree;
    ...
    public void OnStateEntered(GameplayState state)
    {
        ...
        _window = UIManager.CreateWindow<OE14SkillWindow>();
        LayoutContainer.SetAnchorPreset(_window, LayoutContainer.LayoutPreset.CenterTop);

        CommandBinds.Builder
            .Bind(OE14ContentKeyFunctions.OE14OpenSkillMenu,
                InputCmdHandler.FromDelegate(_ => ToggleWindow()))
            .Register<OE14SkillUIController>();
    ...
    public void OnStateExited(GameplayState state)
    {
        ...
        CommandBinds.Unregister<OE14SkillUIController>();
    }

    public void OnSystemLoaded(OE14ClientSkillSystem system)
    {
        system.OnSkillUpdate += UpdateState;
        _playerManager.LocalPlayerDetached += CharacterDetached;
    }

    public void OnSystemUnloaded(OE14ClientSkillSystem system)
    {
        system.OnSkillUpdate -= UpdateState;
        _playerManager.LocalPlayerDetached -= CharacterDetached;
    }
    ...
    private void SelectNode(OE14NodeTreeElement? node)
    {
        if (_window is null)
            return;
        ...
        if (!_proto.TryIndex<OE14SkillPrototype>(node.NodeKey, out var skill))
        {
            DeselectNode();
            return;
        }
    ...
    private void SelectNode(OE14SkillPrototype? skill)
    {
        if (skill is null)
        {
            DeselectNode();
        ...
        _window.SkillPointText.Text =
            Loc.GetString("oe14-skill-menu-learncost", ("type", Loc.GetString(indexedSkillType.Name)));
    ...
    private FormattedMessage GetSkillDescription(OE14SkillPrototype skill)
    {
    ...
    private void UpdateGraphControl()
    {
        ...
        if (!EntityManager.TryGetComponent<OE14SkillStorageComponent>(_targetPlayer, out var storage))
            return;
        ...
        HashSet<OE14NodeTreeElement> nodeTreeElements = new();
        ...
        foreach (var skill in _allSkills)
        {
            ...
            if (!hide)
            {
                var nodeTreeElement = new OE14NodeTreeElement(
                    skill.ID,
    ...
    private void UpdateState(EntityUid player)
    {
        ...
        foreach (var tree in storage.AvailableSkillTrees)
        {
            ...
            var treeButton2 = new OE14SkillTreeButtonControl(indexedTree.Color,
    ...
    private void SelectTree(ProtoId<OE14SkillTreePrototype> tree)
    {
        if (_window == null)
            return;
