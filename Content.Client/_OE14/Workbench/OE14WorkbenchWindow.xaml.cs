...
public sealed partial class OE14WorkbenchWindow : DefaultWindow
{
    ...
    private OE14ClientSkillSystem _skill;

    public event Action<OE14WorkbenchUiRecipesEntry>? OnCraft;
    ...
    private Dictionary<LocId, List<OE14WorkbenchUiRecipesEntry>> _categories = new();
    private List<OE14WorkbenchUiRecipesEntry> _uncategorized = new();

    private OE14WorkbenchUiRecipesState? _cachedState;
    private OE14WorkbenchUiRecipesEntry? _selectedEntry;
    private string _searchFilter = string.Empty;

    private ISawmill Sawmill { get; init; }

    public OE14WorkbenchWindow()
    {
        RobustXamlLoader.Load(this);
        IoCManager.InjectDependencies(this);

        _skill = _entManager.System<OE14ClientSkillSystem>();

        Sawmill = _log.GetSawmill("oe14_workbench_window");
    ...
    private void AddRecipeListToGrid(List<OE14WorkbenchUiRecipesEntry> category, GridContainer gridContainer)
    {
        foreach (var entry in category)
        {
            ...
            var control = new OE14WorkbenchRecipeControl(entry);
    ...
    public void UpdateState(OE14WorkbenchUiRecipesState recipesState)
    {
        ...
        OptionCategories.AddItem(Loc.GetString("oe14-recipe-category-all"), AllCategoryId);
        ...
        var sortedRecipes = recipesState.Recipes
            ...
            .ThenByDescending(e =>
            {
                if (!_prototype.TryIndex(e.ProtoId, out OE14WorkbenchRecipePrototype? recipe))
                    return 0;
            ...
            {
                if (!_prototype.TryIndex(e.ProtoId, out OE14WorkbenchRecipePrototype? recipe) ||
                    !_prototype.TryIndex(recipe.Category, out OE14WorkbenchRecipeCategoryPrototype? category))
                    return string.Empty;
                return category.ID;
            });
        ...
        foreach (var entry in sortedRecipes)
        {
            ...
            {
                entries = new List<OE14WorkbenchUiRecipesEntry>();
        ...
        var sortedCategories = _categories
            .OrderByDescending(c =>
            {
                var categoryProto = _prototype.EnumeratePrototypes<OE14WorkbenchRecipeCategoryPrototype>()
    ...
    private bool ProcessSearchFilter(OE14WorkbenchUiRecipesEntry entry, OE14WorkbenchRecipePrototype indexedEntry)
    {
    ...
    private bool ProcessSearchCategoryFilter(OE14WorkbenchRecipePrototype indexedEntry)
    {
    ...
    private void RecipeSelect(OE14WorkbenchUiRecipesState recipesState)
    {
    ...
    private void RecipeSelect(OE14WorkbenchUiRecipesEntry cachedEntry)
    {
        if (_cachedState is null)
            return;
    ...
    private void RecipeSelect(OE14WorkbenchUiRecipesEntry entry, OE14WorkbenchRecipePrototype recipe)
    {
