...
public sealed partial class OE14TradingPlatformWindow : DefaultWindow
{
    ...
    private OE14TradingPlatformUiState? _cacheState;
    private Entity<OE14TradingReputationComponent>? _cachedUser;
    private Entity<OE14TradingPlatformComponent>? _cachedPlatform;

    private IEnumerable<OE14TradingPositionPrototype> _allPositions = [];

    private ProtoId<OE14TradingFactionPrototype>? _selectedFaction;
    private OE14TradingPositionPrototype? _selectedPosition;
    public event Action<ProtoId<OE14TradingPositionPrototype>>? OnBuy;

    private ISawmill Sawmill { get; init; }

    public OE14TradingPlatformWindow()
    {
        RobustXamlLoader.Load(this);
        IoCManager.InjectDependencies(this);

        Sawmill = _log.GetSawmill("oe14_trading");
    ...
    private void SelectNode(OE14NodeTreeElement? node)
    {
        ...
        if (!_proto.TryIndex<OE14TradingPositionPrototype>(node.NodeKey, out var indexedPosition))
            return;

        SelectNode(indexedPosition);
    }

    private void SelectNode(OE14TradingPositionPrototype? node)
    {
        ...
        BuyPriceHolder.AddChild(new OE14PriceControl((int)price));
    }
    ...
    {
        _allPositions = _proto.EnumeratePrototypes<OE14TradingPositionPrototype>();
        UpdateGraphControl();
    }

    public void UpdateState(OE14TradingPlatformUiState state)
    {
        _cacheState = state;
        ...
        if (!_e.TryGetComponent<OE14TradingPlatformComponent>(plat, out var platComp))
            return;
    ...
    private void UpdateGraphControl()
    {
        ...
        foreach (var position in _allPositions)
        {
            ...
            var node = new OE14NodeTreeElement(position.ID, unlocked, active, new Vector2(position.ReputationLevel.Float() * 50, position.UiPosition * 50) , position.Icon);
        ...
        GraphControl.UpdateState(
            new OE14NodeTreeUiState(
                ...
                frameIcon: new SpriteSpecifier.Rsi(
                    new ResPath("/Textures/_OE14/Interface/NodeTree/trading.rsi"),
                    "frame"),
    ...
    private void SelectFaction(OE14TradingFactionPrototype faction)
    {
        ...
        TreeName.Text = Loc.GetString("oe14-trading-faction-prefix") + " " + Loc.GetString(faction.Name);
        UpdateGraphControl();
    }
}
