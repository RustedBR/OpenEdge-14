...
public sealed class OE14IdentityRecognitionBoundUserInterface : BoundUserInterface
{
    [Dependency] private readonly IEntityManager _entManager = default!;
    [Dependency] private readonly IPlayerManager _player = default!;

    [ViewVariables]
    private OE14RememberNameWindow? _window;

    private NetEntity? _rememberedTarget;

    public OE14IdentityRecognitionBoundUserInterface(EntityUid owner, Enum uiKey) : base(owner, uiKey)
    {
    ...
    protected override void Open()
    {
        ...
        _window = this.CreateWindow<OE14RememberNameWindow>();
    ...
    private void OnLabelChanged(string newLabel)
    {
        ...
        SendPredictedMessage(new OE14RememberedNameChangedMessage(newLabel, _rememberedTarget.Value));
    }
    ...
    private string? CurrentName()
    {
        ...
        if (!_entManager.TryGetComponent<MindContainerComponent>(_player.LocalEntity, out var mindContainer))
            return null;
        if (!_entManager.TryGetComponent<OE14RememberedNamesComponent>(mindContainer.Mind, out var knownNames))
            return null;
    ...
    protected override void UpdateState(BoundUserInterfaceState state)
    {
        ...
        switch (state)
        {
            case OE14RememberNameUiState rememberNameUiState:
